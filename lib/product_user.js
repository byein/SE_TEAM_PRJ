                ...
                if(error) throw error;
                        ...
                        db.query(`select sum(bQuantity) sum from basket b, member m where b.member_id=m.mId and b.product_id=? and m.mId=? GROUP BY b.product_id;`, [filteredId, request.session.ID], function(error2, basket){
                            if (error2) throw error2;
                                ...
                                db.query(`SELECT * FROM review WHERE product_id =?`, [filteredId], function(error3, review){
                                    db.query(`SELECT mPost_Code, mRoad_address, mJibun_address, mDetail_address FROM member WHERE mId=?`, request.session.ID, function(error, address)
                                    {   if (address == undefined){
                                                    address =[];}

                                            console.log(address);
                                        db.query(`SELECT ROUND(AVG(rPoint),2)  average FROM review WHERE product_id=?`, [filteredId], function(error1, avg, fields){
                                            if(request.session.is_logined == true){
