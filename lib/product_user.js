...
exports.detail = function(request, response){
        ...
        db.query(`SELECT * FROM product WHERE pIdx=?`, [filteredId], function(error, product){
                if(error) throw error;
                        ...
                        db.query(`select sum(bQuantity) sum from basket b, member m where b.member_id=m.mId and b.product_id=? and m.mId=? GROUP BY b.product_id;`, [filteredId], function(error2, basket){
                                if(request.session.is_logined == true){
